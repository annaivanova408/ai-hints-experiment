...
function HintTrial({item, condition, settings, demo, practice=false, optionOrder, onDone, emit, save}) {
  ...
  if(stage==="question") return <Frame eyebrow={practice?"Тренировочное задание":"Вопрос по фрагменту"}><div className="answer-content"><h2>{item.stem}</h2><div className="options">{keys.map((key,index)=><button key={key} className={choice===key?"selected":""} onClick={()=>setChoice(key)}><span>{index+1}</span>{item.options[key]}</button>)}</div><div className="answer-actions"><button className="primary" onClick={()=>{emit(33,"hint_request",{segment_id:item.id,condition});emit(34,"hint_onset",{segment_id:item.id,condition});enter("hint")}}><Lightbulb size={20}/> Нужна подсказка</button><span>Подсказку необходимо открыть перед подтверждением ответа</span></div></div></Frame>;
